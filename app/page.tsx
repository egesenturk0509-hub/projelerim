...
void dur() {
  ...
  },
  {
    ...
    materials: ['Arduino UNO', 'MQ-2 Gaz Sensörü', 'Buzzer', '16x2 LCD Ekran', 'LED'],
  ...
  return (
    <div>
      ...
      <header>
        ...
        <div className="header-sub">Ege Şentürk | Geleceğin Teknolojileri</div>
      </header>
      ...
      <div className="container">
        {view === 'grid' && (
          <div className="grid">
            {PROJECTS.map((proj) => (
              <div key={proj.id} className="card" onClick={() => { setSelectedProject(proj); setView('detail'); }}>
        ...
        {view === 'detail' && selectedProject && (
          <div className="detail-view">
            <button className="btn-back" onClick={() => setView('grid')}>⬅ Geri Dön</button>
            <h1>{selectedProject.icon} {selectedProject.title}</h1>
            <p>{selectedProject.description}</p>
            <h3>📦 Gerekli Malzemeler</h3>
            <ul>{selectedProject.materials.map((m: string, i: number) => <li key={i}>{m}</li>)}</ul>
            <h3>🔌 Bağlantı Şeması</h3>
            <div className="code-area" style={{background: '#f0f0f0', color: '#333'}}>{selectedProject.connections}</div>
            <h3>💻 Arduino Kodu <button className="btn-download" onClick={() => downloadFile(`${selectedProject.id}_kod.ino`, selectedProject.code)}>İndir</button></h3>
            <div className="code-area">{selectedProject.code}</div>
          </div>
        )}
      </div>
    </div>
